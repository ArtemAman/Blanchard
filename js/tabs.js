window.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.catalog__button').forEach(function (tabsBtn) {
    tabsBtn.addEventListener('click', function (event) {
      ...
      document.querySelectorAll('.catalog__button').forEach(function (tabborder) {
        tabborder.classList.remove('country-active')
      })
      document.querySelector(`[data-target="${path}"]`).classList.add('tab_content_active')
      document.querySelectorAll('.catalog__button').forEach(function (tabborderadd) {
        tabborderadd.addEventListener('click', function (event2) {
          event2.target.classList.add('country-active')
        })
      })
    })
  });
